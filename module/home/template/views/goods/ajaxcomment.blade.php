    ...
    <div class="deta-descri p">
        <div class="person-ph-name">
            <div class="per-img-n p">
                @if($v['is_anonymous'] == 1) 
                    <div class="img-aroun"><img src="/assets/static/images/headPic.jpg"/></div>
                    <div class="menb-name"> 匿名用户</div>
                @else
                    <div class="img-aroun"><img src="{{ $v['head_pic'] ?: '/assets/static/images/headPic.jpg' }}"/></div>
                    <div class="menb-name">{{ getsubstr($v['username'],0,6) }}</div>
                 @endif 
            </div>
...
@endforeach
            ...
            <!--购买商品规格-->
                <!--<span>{{ htmlspecialchars_decode($v['spec_key_name']) }}</span>-->
            <!--评论者评论时间与购买时间差-->
                <!--<span>购买{{ round(($v['add_time']-$v['pay_time'])/3600/24) }}天后评价</span>-->
    ...
    <!--点赞，回复-->
        <div class="g_come">
           <!-- <a href="javascript:void(0);"><i class="detai-ico c-cen"></i>{{ count((array)$replyList[$v['comment_id']]) }}</a>-->
...
<!--商家回复-s-->
    <div class="comment-replylist">
        @foreach( $replyList[$v['comment_id']] ?: [] as $k => $v3 )
            ...
            <div class="comment-reply-item hide" data-vender="0" data-name="{{ $reply_list['user_name'] }}" data-uid="" style="display: block;">
                <div class="reply-infor clearfix">
                    ...
                    <div class="side">
                        <span class="date">{{ date('Y-m-d H:i:s',$v3['add_time']) }}</span>
                    </div>
                </div>
                <!--回复商家内容-s-->
