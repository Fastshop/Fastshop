...
class WechatLogic
{
    ...
    private function createShareQrCode($backImg, $qrText, $headPic)
    {
        ...
        if($backImg && is_file($backImg)) {
            ...
            $QR_height = $QR_width;
            $QR->thumb($QR_width, $QR_height, Image::THUMB_CENTER)->save($qr_code_file, NULL, 100);
            $back->thumb($backWidth, $backHeight, Image::THUMB_CENTER)
                ->water($qr_code_file, Image::WATER_CENTER, 90)->save($qr_code_file, NULL, 100);
            $QR = $back;
        }
        ...
        if($headPic) {
            ...
            if(file_exists($headPic)) {
                ...
                $QR = $QR->water($logo_file, Image::WATER_CENTER);
                unlink($logo_file);
            }
    ...
    private function handleSubscribeEvent($msg)
    {
        ...
        if( !Db::name('oauth_users')->where('openid', $openid)->find()) {
            ...
            } elseif( !$is_bind_account) {//非绑定账号才给注册
                //此处注册也送积分
