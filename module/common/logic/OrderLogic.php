...
class OrderLogic
{
    ...
    public function recordRefundOrder($user_id, $order_id, $user_note, $consignee, $mobile)
    {
        ...
        Db::name('rebate_log')->where(['order_id' => $order_id])->save(['status' => 4, 'confirm_time' => time()]);
        $order = new Order();
    ...
    function make_virtual_code($order)
    {
        ...
        $message_logic = new MessageLogisticsLogic([]);
    ...
    function order_pay_cancel($order_id)
    {
        ...
        Db::name('order')->where("order_id=$order_id")->save(['pay_status' => 0]);
        ...
        //分销设置
        Db::name('rebate_log')->where("order_id = {$order['order_id']}")->save(['status' => 0]);
    }
    ...
    public function deliveryHandle($data)
    {
        ...
        if($res && $res['status'] == 1) {
            ...
            if($users) {
                ...
                $params = ['user_name' => $nickname, 'consignee' => $data['consignee']];
        ...
        if($r) {
            ...
            return ['status' => 1, 'printhtml' => isset($result['printhtml']) ? $result['printhtml'] : ''];
        } else {
    ...
    public function submitOrderExpress($data, $orderGoods)
    {
        ...
        $region_ids = [$shop_info['province'], $shop_info['city'], $shop_info['district'], $data['province'], $data['city'], $data['district']];
        $region = Db::name('region')->where(['id' => ['in', $region_ids]])->getField('id,name');
    ...
    {
        $p = Db::name('region')->where(['id' => $p])->field('name')->find();
        $c = Db::name('region')->where(['id' => $c])->field('name')->find();
        $d = Db::name('region')->where(['id' => $d])->field('name')->find();
    ...
    public function disposereRurnOrderCoupon($return_info)
    {
        ...
        if( !empty($coupon_info)) {
            if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
                $coupon = Db::name('coupon')->where(['id' => $coupon_info['cid']])->find();
                ...
                Db::name('coupon_list')->where(['id' => $coupon_info['id']])->delete();
                Db::name('coupon')->where(['id' => $coupon_info['cid']])->setDec('send_num');
            }
        }
    }
    ...
    public function getRefundGoodsMoney($return_goods)
    {
        ...
        if($return_goods['is_receive'] == 1) {
            ...
            $coupon_info = Db::name('coupon_list')->where(['uid' => $return_goods['user_id'], 'get_order_id' => $return_goods['order_id']])->find();
            if( !empty($coupon_info)) {
                if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
                    $coupon = Db::name('coupon')->where(['id' => $coupon_info['cid']])->find();
