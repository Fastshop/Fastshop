...
class UsersLogic extends Model
{
    ...
    public function thirdLogin($data = [])
    {
        if( !$data['openid'] || !$data['oauth']) {
            return ['status' => -1, 'msg' => '参数有误openid或oauth丢失', 'result' => 'aaa'];
        }
        $user2 = session('user');
        if( !empty($user2)) {
            $r = $this->oauth_bind($data);//绑定账号
            if($r) {
                return ['status' => 1, 'msg' => '绑定成功', 'result' => $user2];
            } else {
                return ['status' => -1, 'msg' => '您的' . $data['oauth'] . '账号已经绑定过账号', 'bind_status' => 0];
            }
        }
    ...
    public function checkUserQrcode($user)
    {
        ...
        if( !$qrcode) {
            ...
            $rs = Db::name("users")->where(['user_id' => $user])->setField("xcx_qrcode", '/' . $newFilePath);
            if($rs)
                $qrcode = '/' . $newFilePath;
    ...
    public function reg($username, $password, $password2, $push_id = 0, $invite = [], $nickname = "", $head_pic = "")
    {
        ...
        if( !$UserRegValidate->scene('reg')->check($data)) {
            return ['status' => -1, 'msg' => $UserRegValidate->getError(), 'result' => ''];
        }
    ...
    public function add_comment($add)
    {
        ...
        if($goods) return ['status' => -1, 'msg' => '您已经评论过该商品', 'result' => ''];
        if($add['goods_rank'] < 1 || $add['service_rank'] < 1) {
            return ['status' => -1, 'msg' => '请给商品服务评分', 'result' => ''];
        }
    ...
    {
        $check = Db::name('order')->where(['order_id' => $order_goods['order_id']])->find();
        //虚拟订单
        if($check['prom_type'] == 5) {
            $integral = Db::name('order_goods')->where(['rec_id' => $order_goods['rec_id'], 'order_id' => $order_goods['order_id']])->find();
