...
class CommentLogic extends Model
{
    ...
    {
        $comment_info = M('comment')->where(['comment_id' => $comment_id])->find();
        $reply = $this->getReplyPage($comment_id);
        return ['comment_info' => $comment_info, 'reply' => $reply];
    }
     ...
     * @return array
     */
    private function getReplyList($comment_id, $parent_id = 0, &$result = [])
    {
        $reply_where = [
    ...
    public function addComment($data)
    {
        ...
        $add['add_time'] = time();
        $add['ip_address'] = Request::instance()->ip();
        $add['zan_num'] = 0;
        $add['parent_id'] = 0;
        $add['is_show'] = 1;
