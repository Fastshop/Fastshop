...
class GoodsLogic extends Model
{
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        Db::name('goods_collect')->add(['goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()]);
        ...
        if($keys) {
            ...
            foreach($filter_spec2 as $key => $val) {
                if($terminal == "pc") {
                    ...
                    $filter_spec[ $val['name'] ]["spec_value"][] = [
                        'item_id' => $val['id'],
                        'item' => $val['item'],
                        'src' => $specImage[ $val['id'] ],
                    ];
    ...
    function getRegionList()
    {
        ...
        if( !empty($res))
            return $res;
        $parent_region = M('region')->field('id,name')->where(['level' => 1])->cache(TRUE)->select();
        $ip_location = [];
        $city_location = [];
        foreach($parent_region as $key => $val) {
            $c = M('region')->field('id,name')->where(['parent_id' => $parent_region[ $key ]['id']])->order('id asc')->cache(TRUE)->select();
    ...
    function getParentRegionList($cid)
    {
        //$pids = '';
        $pids = [];
        $parent_id = M('region')->where(['id' => $cid])->getField('parent_id');
        if($parent_id != 0) {
            //$pids .= $parent_id;
