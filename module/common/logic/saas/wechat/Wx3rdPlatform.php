...
{
    const AUTH_EVENT_ALL = 0; //实例
    const AUTH_EVENT_VERIFY_TICKET = 1; //第三方平台配置
    //授权事件类型
    const AUTH_EVENT_UNAUTHORIZED = 2; //全局事件
    const AUTH_EVENT_AUTHORIZED = 3; //开放平台的ticket推送通知,微信会10分钟推一次
    const AUTH_EVENT_UPDATE_AUTHORIZED = 4; //取消授权事件
    const COMMON_EVENT_ALL = 0; //授权事件
    const COMMON_EVENT_WEAPP_AUDIT_SUCCESS = 1; //更新授权事件
    //普通事件类型
    const COMMON_EVENT_WEAPP_AUDIT_FAIL = 2; //全局事件
    static private $install = NULL; //小程序审核成功事件
    private $config = []; //小程序审核失败事件
