...
class VirtualLogic extends Model
{
    ...
    public function check_virtual_goods($goods_id, $item_id = 0, $goods_num = 0)
    {
        ...
        if($goods['is_virtual'] == 1 && $goods['virtual_indate'] > time() && $goods['store_count'] > 0) {
            ...
            if($item_id > 0) {
                $specGoodsPrice = M('SpecGoodsPrice')->where(['item_id' => $item_id])->cache(TRUE, TPSHOP_CACHE_TIME)->find(); // 获取商品对应的规格价钱 库存 条码

                if($specGoodsPrice) // 有选择商品规格
                {
                    if($specGoodsPrice['store_count'] < $goods_num) {
                        return ['status' => -1, 'msg' => '该商品规格库存不足'];
                    }
            ...
            return ['status' => -1, 'msg' => '该商品不允许购买，原因可能：商品下架、不存在、过期等'];
        }
    }
    ...
    {
        $vrOrders = Db::name('vr_order_code')->where(['order_id' => $order['order_id']])->select();
