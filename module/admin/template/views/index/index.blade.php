	...
	<div class="admincp-header-r">
		<div class="manager">
			...
			<div class="manager-menu">
                <div class="title">
                    <h4>最后登录</h4>
                    <a href="javascript:void(0);" onClick="CUR_DIALOG = ajax_form('modifypw', '修改密码', '{{ U('Admin/modify_pwd',array('admin_id'=>$admin_info['admin_id'])) }}');" class="edit-password">修改密码</a> 
                </div>
                <div class="login-date"> {{ date('Y-m-d H:i:s',session('last_login_time')) }} <span>(IP: {{ session('last_login_ip') }} )</span> </div>
                <div class="title">
                    <h4>常用操作</h4>
...
<div class="admincp-container-left">
    ...
    <div id="admincpNavTabs_{{ $mk }}" class="nav-tabs">
	    ...
	    <dl>
		    <dt><a href="javascript:void(0);"><span class="ico-{{ $mk }}-{{ $key }}"></span><h3>{{ $v2['name'] }}</h3></a></dt>
		    <dd class="sub-menu">
			    <ul>
			    	@foreach( $v2['child'] ?: [] as $key => $v3 )

				    	<li><a href="javascript:void(0);" data-param="{{ $v3['act'] }}|{{ $v3['op'] }}">{{ $v3['name'] }}</a></li>
...
@endforeach 
    <div class="about" title="关于系统" onclick="javascript:layer.open({type: 2,title: '关于我们',shadeClose: true,shade: 0.3,area: ['50%', '60%'],content:'{{ U("Index/about") }}', });"><i class="fa fa-copyright"></i><span>tpshop.cn</span></div>
</div>
