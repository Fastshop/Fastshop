    ...
    <form method="post" enctype="multipart/form-data" name="form1" action="{{ U('System/handle') }}">
        ...
        <div class="ncap-form-default">
            <dl class="row">
                ...
                <dd class="opt">
                    <div class="onoff">
                        <label for="switch1" class="cb-enable  @if($config['cash_open'] == 1) selected @endif ">开启</label>
                        <label for="switch0" class="cb-disable @if($config['cash_open'] == 0) selected @endif ">关闭</label>
                        <input type="radio" onclick="$('#switch_on_off').show();"  id="switch1"  name="cash_open" value="1" @if($config['cash_open'] == 1) checked="checked" @endif >
                        <input type="radio" onclick="$('#switch_on_off').hide();" id="switch0" name="cash_open" value="0" @if($config['cash_open'] == 0) checked="checked" @endif  >
                    </div>
