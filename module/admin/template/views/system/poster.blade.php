...
<script>
    ...
    var data = {
        id:"{{ $poster['id'] ?: 0 }}",
        poster_name:"",                                                       //海报名称
        canvas_width: "{{ $poster['canvas_width'] ?: 375 }}",                   //画布宽度
        canvas_height: "{{ $poster['canvas_height'] ?: 675 }}",                 //画布高度
        poster_width: "{{ $poster['poster_width'] ?: 0 }}",                     //海报宽度
        poster_height: "{{ $poster['poster_height'] ?: 0 }}",                   //海报高度
        back_url: "{{ $poster['back_url'] ?: '' }}",                            //海报路径
        canvas_x: "{{ $poster['canvas_x'] ?: 0 }}",                             //canvas移动的x轴
        canvas_y: "{{ $poster['canvas_y'] ?: 0 }}",                             //canvas移动的y轴
        enabled: 1,                                                           //是否启用海报
    }
