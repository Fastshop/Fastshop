...
class Query
{
    ...
    public function find($data = null)
    {
        ...
        } else if( !empty($options['cache']) && TRUE === $options['cache']['key'] && is_string($pk) && isset($options['where']['AND'][ $pk ])) {
            ...
            if(isset($options['order'])) {
                ...
                foreach($options['order'] as $key => $val) {
                    if(is_numeric($key)) {
                        ...
                        } else if(array_key_exists($val, $options['map'])) {
                            $options['order'][ $options['map'][ $val ] ] = 'asc';
                            unset($options['order'][ $key ]);
                        }
                    } else if(array_key_exists($key, $options['map'])) {
    ...
    protected function getFieldBindType($type)
    {
        if(0 === strpos($type, 'set') || 0 === strpos($type, 'enum')) {
            $bind = PDO::PARAM_STR;
        } else if(preg_match('/(int|double|float|decimal|real|numeric|serial|bit)/is', $type)) {
            $bind = PDO::PARAM_INT;
        } else if(preg_match('/bool/is', $type)) {
            $bind = PDO::PARAM_BOOL;
        } else {
            $bind = PDO::PARAM_STR;
        }
    ...
    protected function parsePkWhere($data, &$options)
    {
        ...
        } else if(is_array($pk) && is_array($data) && !empty($data)) {
            // 根据复合主键查询
            foreach($pk as $key) {
                if(isset($data[ $key ])) {
    ...
    protected function getCacheKey($value, $options, $bind = [])
    {
        if(is_scalar($value)) {
            $data = $value;
        } else if(is_array($value) && is_string($value[0]) && 'eq' == strtolower($value[0])) {
            $data = $value[1];
        }
        if(isset($data)) {
    ...
    protected function parseWhereExp($logic, $field, $op, $condition, $param = [])
    {
        ...
        if(is_string($field) && preg_match('/[,=\>\<\'\"\(\s]/', $field)) {
            ...
            $where[ $field ] = $param;
        } else if(in_array(strtolower($op), ['null', 'notnull', 'not null'])) {
            // null查询
            $where[ $field ] = [$op, ''];
            $this->options['multi'][ $logic ][ $field ][] = $where[ $field ];
    ...
    protected function lazyWrite($type, $guid, $step, $lazyTime)
    {
        ...
        } else if($_SERVER['REQUEST_TIME'] > Cache::get($guid . '_time') + $lazyTime) {
            // 删除缓存
    ...
    public function update(array $data = [])
    {
        ...
        } else if( !isset($key) && is_string($pk) && isset($options['where']['AND'][ $pk ])) {
            $key = $this->getCacheKey($options['where']['AND'][ $pk ], $options, $this->bind);
        }
        ...
        if($options['fetch_sql']) {
            ...
            if(isset($key) && Cache::get($key)) {
                // 删除缓存
                Cache::rm($key);
            } else if( !empty($options['cache']['tag'])) {
            ...
            if($result) {
                if(is_string($pk) && isset($where[ $pk ])) {
                    $data[ $pk ] = $where[ $pk ];
                } else if(is_string($pk) && isset($key) && strpos($key, '|')) {
                    list($a, $val) = explode('|', $key);
                    $data[ $pk ] = $val;
                }
                $options['data'] = $data;
    ...
    public function removeOption($option = TRUE)
    {
        if(TRUE === $option) {
            $this->options = [];
        } else if(is_string($option) && isset($this->options[ $option ])) {
    ...
    public function select($data = null)
    {
        ...
        } else if('collection' == $this->connection->getConfig('resultset_type')) {
            // 返回Collection对象
    ...
    public function with($with)
    {
        ...
        foreach($with as $key => $relation) {
            ...
            if($relation instanceof Closure) {
                ...
                $relation = $key;
            } else if(is_string($relation) && strpos($relation, '.')) {
                $with[ $key ] = $relation;
    ...
    public function order($field, $order = null)
    {
        if( !empty($field)) {
            ...
            } else if( !empty($this->options['via'])) {
                foreach($field as $key => $val) {
                    if(is_numeric($key)) {
