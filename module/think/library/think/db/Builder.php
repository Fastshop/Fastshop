...
abstract class Builder
{
    ...
    protected function parseWhereItem($field, $val, $rule = '', $options = [], $binds = [], $bindName = null)
    {
        ...
        if(in_array($exp, ['=', '<>', '>', '>=', '<', '<='])) {
            ...
            // 表达式查询
            $whereStr .= '( ' . $key . ' ' . $value . ' )';
        } else if(in_array($exp, ['NOT NULL', 'NULL'])) {
            // NULL 查询
            $whereStr .= $key . ' IS ' . $exp;
        } else if(in_array($exp, ['NOT IN', 'IN'])) {
            // IN 查询
            if($value instanceof Closure) {
                $whereStr .= $key . ' ' . $exp . ' ' . $this->parseClosure($value);
            } else {
        ...
        } else if(in_array($exp, ['NOT BETWEEN', 'BETWEEN'])) {
            // BETWEEN 查询
        ...
        } else if(in_array($exp, ['NOT EXISTS', 'EXISTS'])) {
            // EXISTS 查询
            if($value instanceof Closure) {
        ...
        } else if(in_array($exp, ['< TIME', '> TIME', '<= TIME', '>= TIME'])) {
            $whereStr .= $key . ' ' . substr($exp, 0, 2) . ' ' . $this->parseDateTime($value, $field, $options, $bindName, $bindType);
        } else if(in_array($exp, ['BETWEEN TIME', 'NOT BETWEEN TIME'])) {
            if(is_string($value)) {
                $value = explode(',', $value);
            }
