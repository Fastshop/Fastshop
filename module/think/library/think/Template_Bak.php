...
//        }
        ...
        if ($template) {
            ...
            static $namespace = [];
            $viewpath = ROOT_PATH . "template/$pre/views";
            if (!isset($namespace[$pre])) {
                \View::addNamespace($pre, $viewpath);
            }

            $tplname = substr(realpath(ROOT_PATH . $template), strlen(realpath(ROOT_PATH . "template/$pre/rainbow")) + 1);
            $cacheFile = "$viewpath/" . str_replace('.html', ".blade.php", $tplname);
            $name = str_replace('/', '.', substr($tplname, 0, -strlen('.html')));


            if (!file_exists($cacheFile) || filemtime($cacheFile) < filemtime($tpl) || $_GET['debug'] == 'tpl') {
...
//            }
            ...
            echo view("$pre::$name")->with($this->data)->render();
...
//            kd($tplname);
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        // $content = preg_replace_callback("/\<\?php\s+(.+?)\?\>/is", function($test) {
        /*    return "<?php ".preg_replace("/\[(\w+)\]/", "['$1']", $test[1])."?>";*/ // }, $content);
    ...
    private function getRegex($tagName)
    {
        ...
        if ('tag' == $tagName) {
            ...
            if (strlen(ltrim($begin, '\\')) == 1 && strlen(ltrim($end, '\\')) == 1) {
                $regex = $begin . '((?:[\$]{1,2}[a-wA-w_]|[\:\~][\$a-wA-w_]|[+]{2}[\$][a-wA-w_]|[-]{2}[\$][a-wA-w_]|\/[\*\/])(?>[^' . $end .
                    ']*))' . $end;
            } else {
                $regex = $begin . '((?:[\$]{1,2}[a-wA-w_]|[\:\~][\$a-wA-w_]|[+]{2}[\$][a-wA-w_]|[-]{2}[\$][a-wA-w_]|\/[\*\/])(?>(?:(?!' . $end .
                    ').)*))' . $end;
            ...
            switch ($tagName) {
                case 'block':
                    if ($single) {
                        $regex = $begin . '(?:' . $tagName . '\b(?>(?:(?!name=).)*)\bname=([\'\"])(?P<name>[\$\w\-\/\.]+)\\1(?>[^' . $end . ']*)|\/' .
                            $tagName . ')' . $end;
                    } else {
                        $regex = $begin . '(?:' . $tagName . '\b(?>(?:(?!name=).)*)\bname=([\'\"])(?P<name>[\$\w\-\/\.]+)\\1(?>(?:(?!' . $end .
                            ').)*)|\/' . $tagName . ')' . $end;
                ...
                case 'literal':
                    if ($single) {
                        ...
                        $regex .= '(?:(?>[^' . $begin . ']*)(?>(?!' . $begin . '(?>' . $tagName . '\b[^' . $end . ']*|\/' . $tagName . ')' . $end . ')' . $begin . '[^' .
                            $begin . ']*)*)';
                        $regex .= '(' . $begin . '\/' . $tagName . $end . ')';
                    } else {
                        $regex = '(' . $begin . $tagName . '\b(?>(?:(?!' . $end . ').)*)' . $end . ')';
                        $regex .= '(?:(?>(?:(?!' . $begin . ').)*)(?>(?!' . $begin . '(?>' . $tagName . '\b(?>(?:(?!' . $end . ').)*)|\/' . $tagName . ')' . $end .
                            ')' . $begin . '(?>(?:(?!' . $begin . ').)*))*)';
                        $regex .= '(' . $begin . '\/' . $tagName . $end . ')';
                    }
                    break;
                ...
                case 'extend':
                    ...
                    if ($single) {
                        $regex = $begin . $tagName . '\b(?>(?:(?!' . $name . '=).)*)\b' . $name . '=([\'\"])(?P<name>[\$\w\-\/\.\:@,\\\\]+)\\1(?>[^' .
                            $end . ']*)' . $end;
                    } else {
                        $regex = $begin . $tagName . '\b(?>(?:(?!' . $name . '=).)*)\b' . $name .
                            '=([\'\"])(?P<name>[\$\w\-\/\.\:@,\\\\]+)\\1(?>(?:(?!' . $end . ').)*)' . $end;
                    }
                    break;
            }
                ...
                foreach ($baseBlocks as $name => $val) {
                    ...
                    if (!empty($children[$name])) {
                        ...
                        foreach ($children[$name] as $key) {
                            $replace = str_replace($baseBlocks[$key]['begin'] . $baseBlocks[$key]['content'] .
                                $baseBlocks[$key]['end'], $blocks[$key]['content'], $replace);
                        }
                    }
                    if (isset($blocks[$name])) {
                        ...
                        if (!empty($val['parent'])) {
                            ...
                            if (isset($blocks[$parent])) {
                                $blocks[$parent]['content'] = str_replace($blocks[$name]['begin'] . $blocks[$name]['content'] .
                                    $blocks[$name]['end'], $replace, $blocks[$parent]['content']);
                            }
    ...
    public function parseVar(&$varStr)
    {
        ...
        if (preg_match_all('/\$[a-zA-Z_](?>\w*)(?:[:\.][0-9a-zA-Z_](?>\w*))+/', $varStr, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            while ($matches[0]) {
                        ...
                        if ('$Think' == $first) {
                            ...
                            switch ($this->config['tpl_var_identify']) {
                                ...
                                case 'obj': // 识别为对象
                                    ...
                                    break;
                                default: // 自动判断数组或对象
                                    $parseStr = '(is_array(' . $first . ')?' . $first . '[\'' . implode('\'][\'', $vars) . '\']:' . $first . '->' .
                                        implode('->', $vars) . ')';
                            }
                        }
                    } else {
                ...
                $varStr = preg_replace('/\[(\w+)\]/is', "['$1']", $varStr);
            }
            unset($matches);
        }
        //print $varStr."\n";
        $varStr = preg_replace('/\[(\w+)\]/is', "['$1']", $varStr);
        // if(preg_match('/\[\w+\]/', $varStr)){
        //     kd($varStr);
        // }
        return;
    }
