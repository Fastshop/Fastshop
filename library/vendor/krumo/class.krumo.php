...
/**
    ...
    public static function dump($data, $second = '') {
        ...
        while($d = array_pop($_)) {
            ...
            if(in_array($function, ["krumo", "k", "kd"]) || (strToLower(@$d['class']) == 'krumo') ||
                (is_callable($callback) && call_user_func($callback, $d))) {
                break;
            }
        }
    ...
    static function base64_encode_image($filename, $filetype = 'png') {
        if($filename) {
...
if( ! function_exists('kd')) {
    function kd(...$_) {
        ...
        $_ = func_get_args();
        call_user_func_array(['krumo', 'dump'], $_);
