...
function tpl_adv($order, $limit, $where, $item, $key, $pid) {
    
    // return [];
    
    $ad_position = M("ad_position")->cache(true, TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height", "position_id");
    $result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < '".strtotime(date('Y-m-d H:00:00')).'\' and end_time > \''.
        strtotime(date('Y-m-d H:00:00'))."' ")->order("orderby desc")->cache(true, TPSHOP_CACHE_TIME)->limit($limit)->select();
    
    if(is_array($ad_position) && ! in_array($pid, array_keys($ad_position)) && $pid) {
        M("ad_position")->insert([
            "position_id"   => $pid,
...
function tpl_query($sql) {
    $md5_key = md5($sql);
    if(empty($result = S("sql_".$md5_key))) {
        $result = \think\Db::query($sql);
        S("sql_".$md5_key, $result);
    }
    return $result;
}
 ...
 * @return mixed
 */
function httpRequest($url, $method = "GET", $postfields = null, $headers = [], $debug = false, $timeout = 60) {
...
function flash_sale_time_space() {
    ...
    $time_space = [
        ...
        '2' => [
            'font'       => date("H:i", $flash_sale_time + $space),
            'start_time' => $flash_sale_time + $space,
            'end_time'   => $flash_sale_time + 2 * $space,
        ],
        '3' => [
            'font'       => date("H:i", $flash_sale_time + 2 * $space),
            'start_time' => $flash_sale_time + 2 * $space,
            'end_time'   => $flash_sale_time + 3 * $space,
        ],
        '4' => [
            'font'       => date("H:i", $flash_sale_time + 3 * $space),
            'start_time' => $flash_sale_time + 3 * $space,
            'end_time'   => $flash_sale_time + 4 * $space,
        ],
        '5' => [
            'font'       => date("H:i", $flash_sale_time + 4 * $space),
            'start_time' => $flash_sale_time + 4 * $space,
            'end_time'   => $flash_sale_time + 5 * $space,
        ],
    ];
    return $time_space;
}
